...
const server = m_Http.createServer((req, res) => {
  ...
  // /redir?code=4bbee220dd68bf8f9dd9
    ...
    const query = m_Url.parse(req.url, true).query;
    say('>>>>>>>>>>>>>>>>>>>>query');
    say(query);
    ...
    if (!gh_code)
      ...
      m_Request.post(
        ...
        (p_error, p_response, p_body) => {
          ...
          say('>>>>>>>>>>>>>>>m_Request.post' + m_Request.post);
          say(error);
          say(error_desc);
          say(access_token);
          say(scope);
          ...
          const tempCookieString = 
            g_serverState.createTempCookie(
              access_token,
              req.headers['host']
             );
          // 302 to prevent caching. Just in case! ;-)
  ...
  else if (req.url == '/access_token') {
    res.writeHead(200, 
      [
        ['Content-type', 'text/plain']
      ]
    );
    res.write(g_serverState.getAccessToken(req.headers.cookie));
    res.end('empty!');
  ...
  if (resourceIndex == -1)
    ...
    const url = 'https://avatars0.githubusercontent.com/u/55109130?s=460&v=4';
    const img = `<img src=${url} width=40 height=40/>`

    m_Fs.readFile(filePath, (err, content) => {
      if (err)
        res.end();
      else {
        res.end(img + content,'utf8');
      }
    });
  }
});
