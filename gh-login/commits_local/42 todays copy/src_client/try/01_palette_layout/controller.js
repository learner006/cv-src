class Controller {
  ...
  callback_MouseDownCanvas(p_MouseEvent) {
    let e = p_MouseEvent;
    this.editor.event_DoMouseDown(e.offsetX, e.offsetY);
  }

  callback_MouseUpCanvas() {
    this.editor.event_DoMouseUp();
  }
  ...
  callback_MouseClickPickNewImageSize(p_el) {
    ...
    this.editorManager.loadName = p_el.id;
  ...
  callback_MouseClickChooseTool(p_el) {
    this.editorManager.event_DoChooseTool(p_el.id);
  }
  callback_MouseClickColorsHistory(p_MouseEvent) {
    let e = p_MouseEvent;
    ...
    //alert(getElementIndex(e.target));
    let listItemIndex = getElementIndex(e.target);


    this.editorManager.setNewColorUsingHistory(listItemIndex);
  }
}