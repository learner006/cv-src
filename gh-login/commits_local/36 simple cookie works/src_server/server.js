...
class ServerState {
  constructor() {
    ...
    this.tempCookie = new StringStorage();
    this.sessionCookie = new StringStorage();

    this.cookieParams = {
      tempCookie : {name: "sessiont", 'max-age': '10s', 'secure-dt-gap' : 10 * 1000},
      sessionCookie: {name: "session", 'max-age': '7d', 'secure-dt-gap' : 7 * 24 * 60 * 60 * 1000}
    }
...
function getTempCookieText() {
  const a = g_ServerState.cookieParams['tempCookie'];
  const val = 'xxx';
