...
class Image {
  ...
  saveToImage(p_imageDest) {
    ...
    for(let yd = 0; yd < destArr.length; ++yd)
      for(let xd = 0; xd < destArr[yd].length; ++xd)
      {
        let srcArr = imageSrc.data;

        let pixelCoordsOrigX = transformX[xd];
        let pixelCoordsOrigY = transformY[yd];
        ...
        for(let srcIdxY = startY; srcIdxY < endY; ++srcIdxY)
          for(let srcIdxX = startX; srcIdxX < endX; ++srcIdxX) {
